...
| EXACT_TREE_PREFLIGHT_V2.json | BYTE_IDENTICAL_COPY | T8_EXACT_TREE_PREFLIGHT_V2 | 9b924077b286ef8c7c57e8a6e757cfb22a9784d2b44e8b328b11a55ba27abfc2 | d88d1dba290f9a86033db7a875f8013b813472c24390c8a2736a09b7670b3a5b |
| DECISION_V2.json | BYTE_IDENTICAL_COPY | T7_TERMINAL_DECISION_V2 | 4e220ecee2a8e3ac957c6b5d89ee79dd4f1920503de7442ea6b2e2e06ac7c882 | 2eb8b3824023b16566d662d63eb5f830368d483ff30e4ee90210b8c2607167d0 |
...
| RAISE_SIZING_FRONTIER_RESOLUTION.json | DIGEST_REFERENCE_NO_BYTES | T1B_RAISE_SIZING_FRONTIER_RESOLUTION_V2 | 9df16dbb741659a8f6983e7ce243e3f1796b9fbf6b71be742fd0bc0b5c05540b | cfdc9887a5e71fdf899bce3512e31e501b4fba08d25d6ec8dec4d8d388f1b01d |
...
`INTEGRATION_REPORT.json` is the machine-readable source of this document; `INTEGRATION_REPORT.md` is rendered from it. `ARTIFACTS.json` binds all 12 members by byte digest and content-addressed object, and `INTEGRATION_REPORT.sha256` pins the report itself.
...
* the preflight tool pins now equal the restored v1 / protocol v2 custody bytes (repaired by 9859c46), so issue419_exact_tree_preflight.py --check is GREEN
* the repaired preflight v2 and terminal decision v2 revisions this bundle copies are the ones commits 9859c46 and 0f9a6eb wrote
...
| `python3 tools/simulation/issue419_exact_tree_preflight.py --check` | PASS: the frozen v1 preflight and the protocol v2 custody row are re-derived from the restored bytes (repaired by 9859c46) | 0 | — |
...
| `PYTHONPATH=. python3 tests/simulation/test_issue419_exact_tree_preflight.py` | OK (24 tests; was RED before the 9859c46 pin repair) | 0 | — |
| `python3 tools/training/build_hierarchical_integration_bundle_v2.py --check` | PASS (members re-derived, no drift) | 0 | — |
| `PYTHONPATH=. python3 tests/training/test_issue419_hierarchical_exact_tree.py` | OK (24 tests; was RED before this regeneration) | 0 | — |
| `python3 tools/training/build_hierarchical_evidence_integrity_correction_v2.py --check` | FAIL: bundle member is stale: EVIDENCE_INTEGRITY_CORRECTION_V2.json (its mutant-name survey re-derives from the live tree; bookkeeping only, no frozen byte moved) | 1 | EVIDENCE_INTEGRITY_CORRECTION_V2_SNAPSHOT_REDERIVATION_DRIFT |
...
* superseded by `analysis/issue419_hierarchical_tree/exact_tree_preflight_v2/EXACT_TREE_PREFLIGHT_V2.json`; follow-on findings `V1_PREFLIGHT_PIN_STALE, PROTOCOL_V2_PREFLIGHT_PIN_STALE` are `RESOLVED` (repaired by `9859c46 (task backlog-911)`)
...
* superseded: `93e7e3ede0a69e6b3e35f40217bd53ff95d1fbbb847ca3fbc9181c0689d0152e` (canonical `29b87a82533a69b083034035dd9e2867e4dcf68bea099324675be5441a8f8174`, kept at `analysis/issue419_hierarchical_tree/raise_sizing_frontiers/RAISE_SIZING_FRONTIER_RESOLUTION.json`)
* superseding: `9df16dbb741659a8f6983e7ce243e3f1796b9fbf6b71be742fd0bc0b5c05540b` (canonical `cfdc9887a5e71fdf899bce3512e31e501b4fba08d25d6ec8dec4d8d388f1b01d`, `analysis/issue419_hierarchical_tree/raise_sizing_frontiers_v2/RAISE_SIZING_FRONTIER_RESOLUTION.json`)
* mechanism: `poker-raise-sizing-frontier-resolution/v2`; thresholds moved: `false`; gates relaxed: `false`

Claim: the v2 revision (commit 0f9a6eb) adds the per-frontier blocker the frozen v1 bundle lacks; the v1 bytes stay byte-frozen custody, re-verified on every build and never rewritten, and the terminal decision v2 binds the v2 revision for its values while recording v1 only as custody
...
## 5. Findings: what is residual and what is resolved

Residual findings (each still blocks `READY_FOR_INTEGRATION` where marked):
...
| EVIDENCE_INTEGRITY_CORRECTION_V2_SNAPSHOT_REDERIVATION_DRIFT | false | the evidence-integrity correction v2 record pins the integration bundle surface it observed, and its own --check re-derives its mutant-name survey from the live tree. Re-binding this bundle to the repaired preflight v2 / decision v2 removes the pre-repair preflight v2 copy that carried two mutant-name mentions, so that survey re-derives with fewer recorded mention paths and the record now reports `bundle member is stale`. The drift is bookkeeping only: the record digest table, tool-pin table, frozen-byte re-verification and order-guard results are byte-identical, and no frozen byte moved. This task may not rewrite a v2 record, so the closure is recorded here and in the decision file instead of editing `analysis/issue419_hierarchical_tree/evidence_integrity_correction_v2/EVIDENCE_INTEGRITY_CORRECTION_V2.json` |

Resolved / closed findings (recorded history, never a live pin):

| finding | status | detail |
| --- | --- | --- |
| V1_PREFLIGHT_PIN_STALE | RESOLVED | the preflight tool once pinned the pre-correction v1 preflight bytes, which the evidence-integrity correction had restored to a different (authoritative) digest, so issue419_exact_tree_preflight.py --check failed closed on its own frozen surface; commit 9859c46 restored the pins, so the pin now equals the restored authority on disk and the check is GREEN at this head |
| PROTOCOL_V2_PREFLIGHT_PIN_STALE | RESOLVED | the same preflight tool also pinned the pre-correction protocol v2 bytes, which tripped the protocol-v2 custody check inside the preflight suite; commit 9859c46 restored both protocol v2 pins to the amended authority, so the suite is GREEN at this head |
| INTEGRATION_BUNDLE_V2_PINS_PRE_REPAIR_REVISIONS | CLOSED_BY_THIS_REGENERATION | the integration bundle v2 bound EXACT_TREE_PREFLIGHT_V2.json and DECISION_V2.json to the pre-repair revisions that existed when it was authored; this regeneration re-binds both copies to the repaired revisions and both members re-verify byte-for-byte |

`V1_PREFLIGHT_PIN_STALE` in detail: the preflight tool pins `V1_PREFLIGHT_SHA256=456d85be57b910d3a56cb0160ba0ba35c988f7f69da9f965f0a887aaac473ae6` and `V1_INDEX_SHA256=97e90eac0a9302f1d0b304fa698f5179c7c0ee98ad52ebe307a911d9ccbfa5be`, and the restored authority on disk is `456d85be57b910d3a56cb0160ba0ba35c988f7f69da9f965f0a887aaac473ae6` / `97e90eac0a9302f1d0b304fa698f5179c7c0ee98ad52ebe307a911d9ccbfa5be` (`pin_matches_on_disk_bytes=true`). `887e46a (task backlog-agg)` re-pointed the pin after the correction `c904524` restored the bytes; `9859c46 (task backlog-911)` then restored the pin, so `python3 tools/simulation/issue419_exact_tree_preflight.py --check` is GREEN again (`check PASS (the frozen v1 preflight is re-derived)`) and the finding no longer blocks integration.

`PROTOCOL_V2_PREFLIGHT_PIN_STALE` in detail: the same tool pins `PROTOCOL_V2_BYTE_SHA256=74b8a006ae84f8b9b22913ef76977e95f08992feb9765639a46d1ac49eb87350` and `PROTOCOL_V2_CANONICAL_PAYLOAD_SHA256=cb598a9fc2353aa78f19a7263a62a8ccbdba60eb400264787e896d0c232f19de`, and the amended protocol v2 authority on disk is `74b8a006ae84f8b9b22913ef76977e95f08992feb9765639a46d1ac49eb87350` / `cb598a9fc2353aa78f19a7263a62a8ccbdba60eb400264787e896d0c232f19de` (`pin_matches_on_disk_bytes=true`). `887e46a (task backlog-agg)` re-pointed both protocol v2 pins to the pre-correction digests and `9859c46 (task backlog-911)` restored them, so the preflight suite is `OK (24 tests)`.

`INTEGRATION_BUNDLE_V2_PINS_PRE_REPAIR_REVISIONS` in detail: the evidence-integrity correction v2 record (`analysis/issue419_hierarchical_tree/evidence_integrity_correction_v2/EVIDENCE_INTEGRITY_CORRECTION_V2.json`, `0d530ad77555986028c15ebe98cb3bda366d68285afca6ad0633e2a0f3f5a177`) recorded this finding and deferred it to the integration-bundle task. This regeneration closes it (`CLOSED_BY_THIS_REGENERATION`, closed by `backlog-hvp (this regeneration)`): before `EXACT_TREE_PREFLIGHT_V2.json`=`2c07ae3279d10ec3e3bbdbe81cdb0d0f1f23e6bbb91cbdadddae113d1a9ea691` / `DECISION_V2.json`=`ac291b9ed363080401e9649a30b157eec7f7b79d5d2dc6df70390ccebc67c4b5`, after `EXACT_TREE_PREFLIGHT_V2.json`=`9b924077b286ef8c7c57e8a6e757cfb22a9784d2b44e8b328b11a55ba27abfc2` / `DECISION_V2.json`=`4e220ecee2a8e3ac957c6b5d89ee79dd4f1920503de7442ea6b2e2e06ac7c882`. It touches no frozen byte.

## 6. Coherence with the T1/T3/T4 authorities

| cross-check | value |
| --- | --- |
| upstream_bundles_verified | 16 |
...
| terminal_decision_v2_digest | 4e220ecee2a8e3ac957c6b5d89ee79dd4f1920503de7442ea6b2e2e06ac7c882 |
| exact_tree_preflight_v1_digest | 456d85be57b910d3a56cb0160ba0ba35c988f7f69da9f965f0a887aaac473ae6 |
| exact_tree_preflight_v2_digest | 9b924077b286ef8c7c57e8a6e757cfb22a9784d2b44e8b328b11a55ba27abfc2 |
| raise_sizing_frontier_v1_digest | 93e7e3ede0a69e6b3e35f40217bd53ff95d1fbbb847ca3fbc9181c0689d0152e |
| raise_sizing_frontier_v2_digest | 9df16dbb741659a8f6983e7ce243e3f1796b9fbf6b71be742fd0bc0b5c05540b |
...
* no hand history, decision row or dataset file was opened: `dataset_or_hand_history_opens = []` over 104 benign evidence files opened
* every frozen input is re-verified byte-for-byte before and after the build: `protected_files_unchanged=true` over 36 files
...
1. the `V1_PREFLIGHT_PIN_STALE` and `PROTOCOL_V2_PREFLIGHT_PIN_STALE` defects fixed by their owner task -- **done**: `9859c46` restored the four pins without moving a frozen surface, so `issue419_exact_tree_preflight.py --check` and the preflight suite are GREEN and this bundle now binds the repaired revisions;
