...
The superseded v1 preflight bundle (`analysis/issue419_hierarchical_tree/exact_tree_preflight`, `456d85be57b910d3a56cb0160ba0ba35c988f7f69da9f965f0a887aaac473ae6`) is the restored-custody v1 revision: byte-pinned, re-verified byte-for-byte on every run and never rewritten. Its pin, the frozen bytes persisted on disk and the frozen v2 protocol `v1_custody` row (`456d85be57b910d3a56cb0160ba0ba35c988f7f69da9f965f0a887aaac473ae6`) are one and the same digest, enforced by the single-source agreement recorded in `frozen_v1_preflight.pin_single_source`. A byte-identical regeneration stays impossible by construction because the v1 payload embeds the `code.preflight_tool.sha256` of the superseded v1 tool, which no longer exists on disk. The v2 bundle in `analysis/issue419_hierarchical_tree/exact_tree_preflight_v2/` supersedes v1 and is the only revision this tool writes (`--revision v2`; `--revision v1` is refused).
