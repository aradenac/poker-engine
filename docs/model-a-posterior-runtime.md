...
| `status = AVAILABLE` with no matched conditioning action while the kept imported/source prior is not the full-support uniform prior (non-uniform, or uniform over a strict subset) | `source_prior_unconditioned` |
...
action is matched but the kept source prior is not the full-support uniform prior
(non-uniform weights, or uniform over a strict subset of the legal combos), the
state is the
