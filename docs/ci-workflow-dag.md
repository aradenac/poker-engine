...
Active workflows: **31**; jobs: **50**; cost proxy: **279**.
...
| `.github/workflows/dataset-integrity.yml` | dataset_population | push, pull_request, workflow_dispatch | 1 | READ_ONLY | REPRO_HELPER_VERIFIED | `${{ github.workflow }}-${{ github.event.pull_request.number || github.ref }}` / cancel=true |
| `.github/workflows/full-hand-arena.yml` | simulation_benchmark | pull_request, workflow_dispatch | 2 | ARTIFACT_ONLY | REPRO_HELPER_VERIFIED | none |
| `.github/workflows/full-hand-protocol.yml` | simulation_benchmark | pull_request, push, workflow_dispatch | 1 | READ_ONLY | REPRO_HELPER_VERIFIED | none |
| `.github/workflows/game-core.yml` | simulation_benchmark | pull_request, workflow_dispatch | 1 | UNKNOWN | REPRO_NOT_VERIFIED | none |
| `.github/workflows/hero-calculated-range-export.yml` | hero_preflop | pull_request, workflow_dispatch | 1 | ARTIFACT_ONLY | REPRO_HELPER_VERIFIED | none |
...
| `.github/workflows/model-b-card-aware-runtime.yml` | model_b | push, pull_request | 1 | READ_ONLY | REPRO_HELPER_VERIFIED | none |
| `.github/workflows/model-b-response-audit.yml` | model_b | workflow_dispatch, push | 1 | REPOSITORY_WRITE | REPRO_NOT_VERIFIED | none |
| `.github/workflows/model-b-reveal-aware.yml` | model_b | pull_request, workflow_dispatch | 1 | ARTIFACT_ONLY | REPRO_HELPER_VERIFIED | none |
...
| `.github/workflows/population-pack-catalog.yml` | product_browser_validation | pull_request, workflow_dispatch | 2 | READ_ONLY | REPRO_HELPER_VERIFIED | `population-pack-catalog-${{ github.ref }}` / cancel=true |
...
| `.github/workflows/postflop-response-refit.yml` | validation_or_utility | push, pull_request, workflow_dispatch | 1 | ARTIFACT_ONLY | REPRO_HELPER_VERIFIED | `postflop-continuation-${{ github.ref }}` / cancel=true |
| `.github/workflows/preflop-contract.yml` | preflop_strategy | pull_request, workflow_dispatch | 1 | UNKNOWN | REPRO_NOT_VERIFIED | none |
| `.github/workflows/preflop-grid-evaluator.yml` | preflop_strategy | pull_request, workflow_dispatch | 1 | READ_ONLY | REPRO_HELPER_VERIFIED | none |
| `.github/workflows/preflop-policy169.yml` | preflop_strategy | pull_request, workflow_dispatch | 1 | READ_ONLY | REPRO_HELPER_VERIFIED | `preflop-policy169-${{ github.ref }}` / cancel=true |
| `.github/workflows/preflop-search.yml` | preflop_strategy | push, pull_request | 1 | READ_ONLY | REPRO_HELPER_VERIFIED | `preflop-search-${{ github.ref }}` / cancel=true |
...
| `.github/workflows/release-handoff-contract.yml` | release_promotion | pull_request, push | 1 | READ_ONLY | REPRO_HELPER_VERIFIED | `release-handoff-contract-${{ github.ref }}` / cancel=true |
...
|---|---|---:|---:|---:|---|
| trainer_js | pull_request | 2/4/20 | 2/4/20 | 0 | SUPPORTED_STATIC_SUBSET |
| game_core | pull_request | 5/7/72 | 5/7/72 | 0 | SUPPORTED_STATIC_SUBSET |
| preflop_decision | pull_request | 4/4/24 | 4/4/26 | 0 | SUPPORTED_STATIC_SUBSET |
| dataset_nlhe_100_200 | push | 3/7/54 | 3/7/54 | 0 | UNKNOWN |
| preflop_model | pull_request | 3/4/22 | 3/4/22 | 0 | SUPPORTED_STATIC_SUBSET |
| repro_helper | push | 3/3/17 | 7/8/49 | 2 | UNKNOWN |
| python_version | pull_request | 10/12/81 | 14/16/113 | 0 | UNKNOWN |
| node_version | pull_request | 5/7/47 | 9/11/79 | 0 | SUPPORTED_STATIC_SUBSET |
| release_handoff_tooling | pull_request | 2/2/10 | 2/2/10 | 0 | SUPPORTED_STATIC_SUBSET |
| model_b_training | push | 2/2/10 | 2/2/10 | 0 | SUPPORTED_STATIC_SUBSET |
