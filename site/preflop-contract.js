(function(root,factory){
  ...
  const priceMetrics=({target_total_bb,to_call_bb,pot_before_bb})=>{
    const target=roundBB(finiteNonnegative(target_total_bb,'target_total_bb')),toCall=roundBB(finiteNonnegative(to_call_bb,'to_call_bb')),pot=roundBB(finiteNonnegative(pot_before_bb,'pot_before_bb'));
    const priceToPot=pot<=EPS?(toCall<=EPS?0:null):roundBB(toCall/pot),denom=pot+toCall,potOdds=denom>EPS?roundBB(toCall/denom):0;
    return {target_total_bb:target,price_to_pot_ratio:priceToPot,pot_odds:potOdds};
  };
  ...
  return {SCHEMA,PROBABILITY_SCHEMA,STATE_TIMING,normalizePosition:normPos,sortPositions,normalizeHistory,historyToken,deriveRemainingToAct,familyFromHistory,legalActionsForState,priceMetrics,canonicalKey,contextId,buildContext,v5RuntimeSignature,normalizeActionProbabilities,incumbentV5Passthrough};
});
