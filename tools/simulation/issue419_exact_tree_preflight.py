...
The frozen ``CALIBRATION`` gate is never re-measured here.  The preflight
*attests* it from the published ``validation/VALIDATION_RESULT.json`` bytes,
whose digest the frozen v2 protocol custody table already pins as
``holdout_boundary.validation_result_sha256``: the digest is re-derived from the
persisted bytes, the published gate rows are cross-checked, and the recorded
verdict is evaluated from that frozen measurement.  No holdout is re-opened --
no hand history is parsed, no validation decision row is read, no metric is
recomputed and no threshold is re-selected -- and the attestation is recorded in
``admissibility_protocol.calibration_gate``.  When no evidence can be attested
the gate stays ``GATE_UNEVALUATED_PRE_VALIDATION`` and fails closed: an
unevaluated layer-B gate may never close a node.

The tool writes its bundle to ``analysis/issue419_hierarchical_tree/
exact_tree_preflight_v2/`` (``--revision v2``, the default).  The superseded
``v1`` bundle (``exact_tree_preflight/``) is the restored-custody v1 revision: a
frozen, pinned surface that is re-verified byte-for-byte here and never
rewritten, with ``--revision v1`` refused outright.  Its bytes are the
pre-mutant revision the recorded evidence-integrity correction put back on disk,
so the tool pin, the frozen bytes and the frozen v2 protocol ``v1_custody`` row
are one and the same digest.  v1 cannot be regenerated byte-identically by
construction -- its payload records ``code.provider.sha256`` and
``code.preflight_tool.sha256`` of the superseded sources, and the recorded v1
tool digest no longer exists on disk -- so the frozen bytes are kept and
byte-pinned against the digests the frozen v2 protocol custody table also
records, with the supersession declared factually in
``frozen_v1_preflight.supersession``.
"""
...
# immutable evidence and must hash back to these digests on every run.  These are
# the *restored* custody digests (the pre-mutant revision the recorded
# evidence-integrity correction put back on disk).  The preflight payload is also
# the digest the frozen v2 protocol ``v1_custody`` table pins, so the tool pin, the
# bytes persisted on disk and the protocol custody row are one and the same digest
# (see :func:`verify_custody_pin_agreement`).
...
# Factual supersession declaration.  The v1 preflight bundle is the *restored
# custody revision*: its bytes are the pre-mutant revision the recorded
# evidence-integrity correction put back on disk, and the tool pin above, the
# frozen bytes and the frozen v2 protocol ``v1_custody`` row are one and the same
# digest.  v1 is superseded by the v2 bundle in ``exact_tree_preflight_v2/`` and is
# never rewritten.  A byte-identical re-derivation stays impossible by
# construction: the v1 payload embeds ``code.preflight_tool.sha256`` of the
# superseded v1 tool, and that digest no longer exists anywhere on disk (the tool
# was rewritten into this v2 revision).  The restored v1 revision therefore cannot
# be rebuilt, and it is byte-pinned instead -- re-verified on every run against the
# digests recorded below and against the frozen v2 protocol custody table.
V1_SUPERSESSION = {
    'declaration': (
        'V1_RESTORED_CUSTODY_REVISION_SUPERSEDED_BY_V2: the v1 preflight bundle is the restored '
        'custody revision (preflight 456d85be, index 97e90eac, frozen protocol v2 74b8a006). It '
        'is superseded by the v2 bundle in exact_tree_preflight_v2/, which is the only revision '
        'this tool writes. v1 is byte-pinned and re-verified on every run and is never rewritten: '
        'its payload embeds the code.preflight_tool.sha256 of the superseded v1 tool, which no '
    ...
    'superseding_bundle': 'analysis/issue419_hierarchical_tree/exact_tree_preflight_v2',
    'regeneration_possible': False,
    'regeneration_reason': (
        'the v1 payload embeds the code.preflight_tool.sha256 of the superseded v1 tool, so '
        'replaying the v1 revision cannot reproduce these bytes'
    ),
    'custody_revision': 'RESTORED_PRE_MUTANT_CUSTODY_REVISION',
    'custody_restoration': {
        'correction_id': 'issue419-evidence-integrity-correction-v1',
        'correction_record': 'analysis/issue419_hierarchical_tree/evidence_integrity_correction',
        'mutant_commit': '871e0bd9e66c978932face1accf1aaf22ac9fa1a',
        'restored_preflight_sha256': V1_PREFLIGHT_SHA256,
        'restored_index_sha256': V1_INDEX_SHA256,
        'restored_protocol_v2_byte_sha256': PROTOCOL_V2_BYTE_SHA256,
    },
    'v1_records_provider_sha256': (
        '1cd25c17bc67c16e78d54fc105cf468e92d9dc7b27d4a5b259141715fd34be18'
    ),
    'v1_records_preflight_tool_sha256': (
        '10a19d3ab218223b442873b0e3406cd6045f204285d2d423fdd5279203be1d32'
    ),
}

PROTOCOL_V2_AMENDMENT_ID = 'V2_AMENDMENT_1_CONDITIONAL_NODE_CLOSURE'
V1_CUSTODY_ROLE = 'V1_EXACT_TREE_PREFLIGHT'
NODE_CLOSURE_RULE_ID = 'NODE_CLOSES_IFF_ALL_FROZEN_LAYER_B_GATES_PASS'
NODE_CLOSURE_VALUE = 'CONDITIONAL'

# The frozen v2 revision is authored *after* the VALIDATION read
# (``status = AUTHORED_AFTER_VALIDATION_READ_NO_THRESHOLD_CHANGE``) and pins the
# byte digest of the already-consumed VALIDATION result through
# ``holdout_boundary.validation_result_sha256``.  The frozen layer-B CALIBRATION
# gate is therefore *attested* from those digest-referenced frozen bytes instead
# of being hardcoded open: the preflight re-opens no holdout, re-parses no
# validation decision row, re-runs no metric and re-selects no threshold.
...
V1_VALIDATION_RESULT_CUSTODY_ROLE = 'CONSUMED_VALIDATION_HISTORY'
CALIBRATION_CANDIDATE_DETAIL_KEY = 'candidate_hierarchical'
CALIBRATION_ACTIVE_DETAIL_KEY = 'active_model_a_v5'
CALIBRATION_ABSOLUTE_GATE_ID = 'calibration_absolute'
CALIBRATION_DELTA_GATE_ID = 'calibration_delta_vs_active'
CALIBRATION_EVIDENCE_SOURCE_FROZEN = (
    'FROZEN_V1_VALIDATION_RESULT_BYTES_REFERENCED_BY_DIGEST_NO_NEW_HOLDOUT_READ'
)
CALIBRATION_ATTESTATION_POINTERS = (
    f'/metrics/secondary/calibration_detail/{CALIBRATION_CANDIDATE_DETAIL_KEY}',
    f'/metrics/secondary/calibration_detail/{CALIBRATION_ACTIVE_DETAIL_KEY}',
    f'/gate/gates[{CALIBRATION_ABSOLUTE_GATE_ID}]',
    f'/gate/gates[{CALIBRATION_DELTA_GATE_ID}]',
)
...
def verify_frozen_v1_preflight() -> dict[str, Any]:
    ...
    for name, pinned in expected.items():
        ...
        digests[name] = actual
    # The v1 payload names the provider and the preflight tool it was derived from;
    # the declaration below is only true while the v1 tool digest stays off-disk.
    v1_payload = json.loads((OUTPUT / NAME).read_text())
    recorded_provider = str(v1_payload['code']['provider']['sha256'])
    recorded_tool = str(v1_payload['code']['preflight_tool']['sha256'])
    if recorded_provider != V1_SUPERSESSION['v1_records_provider_sha256']:
        raise PreflightError(
            'the v1 supersession declaration drifted from the frozen v1 provider digest: '
            f"{recorded_provider} != {V1_SUPERSESSION['v1_records_provider_sha256']}"
        )
    if recorded_tool != V1_SUPERSESSION['v1_records_preflight_tool_sha256']:
        raise PreflightError(
            'the v1 supersession declaration drifted from the frozen v1 tool digest: '
            f"{recorded_tool} != {V1_SUPERSESSION['v1_records_preflight_tool_sha256']}"
        )
    live_provider = sha256_file(Path(H.__file__))
    live_tool = sha256_file(SOURCE_PATH)
    regeneration_possible = bool(
        recorded_provider == live_provider and recorded_tool == live_tool
    )
    ...
    return {
        ...
        'v1_code_bindings': {
            'recorded_provider_sha256': recorded_provider,
            'recorded_preflight_tool_sha256': recorded_tool,
            'live_provider_sha256': live_provider,
            'live_preflight_tool_sha256': live_tool,
            'provider_unchanged_since_v1': recorded_provider == live_provider,
            'preflight_tool_unchanged_since_v1': recorded_tool == live_tool,
        },
        'reason': (
...
def load_frozen_v2_protocol() -> dict[str, Any]:
    ...
    custody_pin = next(
        (
            str(row['sha256'])
            for row in protocol.get('v1_custody', {}).get('v1_surfaces', [])
            if row.get('role') == V1_CUSTODY_ROLE
        ),
        None,
    )
    if custody_pin != V1_PREFLIGHT_SHA256:
        raise PreflightError(
            'the frozen protocol v2 does not pin the recorded v1 preflight: '
            f'{custody_pin} != {V1_PREFLIGHT_SHA256}'
        )
    validation_pin = next(
        (
            str(row['sha256'])
            for row in protocol.get('v1_custody', {}).get('v1_surfaces', [])
            if row.get('role') == V1_VALIDATION_RESULT_CUSTODY_ROLE
        ),
        None,
    )
    if validation_pin != V1_VALIDATION_RESULT_SHA256:
        raise PreflightError(
            'the frozen protocol v2 does not pin the consumed v1 VALIDATION result the '
            f'CALIBRATION gate is attested from: {validation_pin} != {V1_VALIDATION_RESULT_SHA256}'
        )
    holdout = protocol['holdout_boundary']
    if holdout['validation_result_sha256'] != validation_pin:
        raise PreflightError(
            'the frozen protocol v2 holdout boundary disagrees with its own custody table about '
            'the consumed VALIDATION result'
        )
    if holdout['revision_authored_after_validation_read'] is not True:
        raise PreflightError('the v2 revision must be authored after the VALIDATION read')
    if holdout['holdout_reopened_by_this_revision'] is not False:
        raise PreflightError('the v2 revision must not re-open the holdout')
    if holdout['validation_consumed_by_this_revision'] is not False:
        raise PreflightError('the v2 revision must not consume VALIDATION for a new claim')
    if holdout['thresholds_selected_after_reading_the_holdout'] is not False:
        raise PreflightError('no threshold may be selected after reading the holdout')
    if holdout['test_consumed'] is not False or holdout['hero_ev_consumed'] is not False:
        raise PreflightError('the v2 revision must keep TEST and Hero EV unconsumed')
    pooling_gate = next(
    ...
    return {
        ...
        'v1_preflight_custody_pin': custody_pin,
        'v1_validation_result_custody_pin': validation_pin,
    }


def verify_custody_pin_agreement() -> dict[str, Any]:
    """One source of truth for every custody pin this tool publishes.

    Every pin must simultaneously equal (a) the tool constant, (b) the SHA256 of
    the frozen bytes persisted on disk, and (c) the matching row of the frozen v2
    protocol ``v1_custody`` table where such a row exists.  Checking the three
    surfaces together means a future edit can no longer move one of them alone:
    a pin that stops describing the frozen bytes -- or a frozen protocol custody
    row that stops describing the pin -- fails here instead of being papered over.
    """
    protocol = json.loads(PROTOCOL_V2_PATH.read_bytes())
    custody_rows = {
        str(row.get('role')): str(row.get('sha256'))
        for row in protocol.get('v1_custody', {}).get('v1_surfaces', [])
    }
    targets = (
        ('V1_PREFLIGHT_SHA256', OUTPUT / NAME, V1_PREFLIGHT_SHA256, V1_CUSTODY_ROLE),
        ('V1_INDEX_SHA256', OUTPUT / INDEX_NAME, V1_INDEX_SHA256, None),
        ('V1_SUMMARY_SHA256', OUTPUT / SUMMARY_NAME, V1_SUMMARY_SHA256, None),
        ('PROTOCOL_V2_BYTE_SHA256', PROTOCOL_V2_PATH, PROTOCOL_V2_BYTE_SHA256, None),
    )
    sources: dict[str, Any] = {}
    for pin_name, path, pinned, role in targets:
        if not path.is_file():
            raise PreflightError(f'the frozen custody surface is missing: {path}')
        actual = sha256_file(path)
        if actual != pinned:
            raise PreflightError(
                f'{pin_name} diverges from the frozen bytes persisted on disk: '
                f'{actual} != {pinned}'
            )
        row_pin = custody_rows.get(role) if role is not None else None
        if role is not None and row_pin is None:
            raise PreflightError(
                f'the frozen v2 protocol custody table lost its {role} row, which '
                f'{pin_name} must agree with'
            )
        if row_pin is not None and row_pin != pinned:
            raise PreflightError(
                f'{pin_name} diverges from the frozen v2 protocol v1_custody row {role}: '
                f'{row_pin} != {pinned}'
            )
        sources[pin_name] = {
            'pin': pinned,
            ...
            'protocol_v2_custody_role': role,
            'protocol_v2_custody_sha256': row_pin,
            'agrees': True,
        }
    return {
        'check': 'custody_pin_single_source_agreement',
        'result': 'PASS',
        'rule': (
            'pin == sha256(frozen bytes persisted on disk) == frozen v2 protocol '
            'v1_custody row (wherever a row exists)'
        ),
        'sources': sources,
    }


def attest_calibration_gate_evidence(protocol_v2: Mapping[str, Any]) -> dict[str, Any]:
    """Attest the frozen CALIBRATION gate from the digest-referenced v1 VALIDATION bytes.

    The frozen v2 revision is ``AUTHORED_AFTER_VALIDATION_READ_NO_THRESHOLD_CHANGE``
    and pins the byte digest of the *already consumed* VALIDATION result
    (``holdout_boundary.validation_result_sha256``).  This preflight therefore
    re-derives that digest from the persisted bytes -- never taken on trust -- and
    attests the frozen absolute/relative calibration measurement from the
    published artifact.

    Nothing about the holdout is re-opened: no hand history is parsed, no
    validation decision row is read, no metric is recomputed and no threshold is
    re-selected.  Every value the gate consumes is cross-checked against the gate
    rows recorded in the very same bytes, so a drifting reference artifact fails
    closed instead of silently passing.
    """
    # ``protocol_v2`` here is the verified loader wrapper, so the frozen protocol
    # document itself lives one level down; both views have already been verified
    # against the pinned byte and canonical-payload digests by
    # :func:`load_frozen_v2_protocol`.
    thresholds = calibration_gate_thresholds(protocol_v2['protocol'])
    pinned = str(protocol_v2['v1_validation_result_custody_pin'])
    payload_bytes = V1_VALIDATION_RESULT_PATH.read_bytes()
    actual = hashlib.sha256(payload_bytes).hexdigest()
    if actual != pinned:
        raise PreflightError(
            'the consumed v1 VALIDATION result drifted from the digest the frozen protocol v2 '
            f'pins: {actual} != {pinned}'
        )
    payload = json.loads(payload_bytes)
    if payload.get('split') != 'VALIDATION':
        raise PreflightError('the calibration attestation is not the VALIDATION split result')
    if payload.get('test_consumed') is not False:
        raise PreflightError('the attested VALIDATION result must leave TEST unconsumed')
    if payload.get('hero_ev_consumed') is not False:
        raise PreflightError('the attested VALIDATION result must leave Hero EV unconsumed')
    if payload.get('protocol_byte_sha256') != PROTOCOL_BYTE_SHA256:
        raise PreflightError('the attested VALIDATION result was measured under another protocol')
    candidate = payload['candidate']
    if str(candidate['candidate_id']) != H.CANDIDATE_ID:
        raise PreflightError('the attested VALIDATION result is about another candidate')
    if str(candidate['canonical_payload_sha256']) != CANDIDATE_CANONICAL_SHA256:
        raise PreflightError('the attested VALIDATION result pins another candidate payload')
    recorded_thresholds = payload['thresholds']
    if float(recorded_thresholds['maximum_absolute_ece']) != thresholds['maximum_absolute_ece']:
        raise PreflightError('the attested VALIDATION result used another absolute ECE threshold')
    if (
        float(recorded_thresholds['maximum_ece_delta_vs_active'])
        != thresholds['maximum_ece_delta_vs_active']
    ):
        raise PreflightError('the attested VALIDATION result used another relative ECE threshold')

    detail = payload['metrics']['secondary']['calibration_detail']
    candidate_detail = detail.get(CALIBRATION_CANDIDATE_DETAIL_KEY)
    active_detail = detail.get(CALIBRATION_ACTIVE_DETAIL_KEY)
    if not candidate_detail or not active_detail:
        raise PreflightError('the attested VALIDATION result has no calibration detail')
    pooled_ece = float(candidate_detail['ece'])
    active_ece = float(active_detail['ece'])
    ece_delta = pooled_ece - active_ece
    bins_meeting = int(candidate_detail['bins_meeting_minimum_support'])
    bins_per_action_class = int(candidate_detail['bins_per_action_class'])
    action_classes = sorted(str(action) for action in candidate_detail['per_action_class'])
    total_bins = bins_per_action_class * len(action_classes)
    claim_supportable = bool(candidate_detail['claim_supportable'])
    if (
        int(candidate_detail['minimum_bin_support'])
        != thresholds['minimum_bin_support_for_a_calibration_claim']
    ):
        raise PreflightError('the attested VALIDATION result used another bin-support minimum')
    if bins_per_action_class != thresholds['bins_per_action_class']:
        raise PreflightError('the attested VALIDATION result used another bin count')

    recorded = {
        str(row['gate']): row
        for row in payload['gate']['gates']
        if isinstance(row, Mapping)
    }
    absolute_row = recorded.get(CALIBRATION_ABSOLUTE_GATE_ID)
    delta_row = recorded.get(CALIBRATION_DELTA_GATE_ID)
    if not absolute_row or not delta_row:
        raise PreflightError('the attested VALIDATION result lost its calibration gate rows')
    absolute_measured = absolute_row['measured']
    delta_measured = delta_row['measured']
    if abs(float(absolute_measured['ece']) - pooled_ece) > 1e-12:
        raise PreflightError('the attested calibration measurement disagrees with its gate row')
    if abs(float(delta_measured['active_ece']) - active_ece) > 1e-12:
        raise PreflightError('the attested active-reference ECE disagrees with its gate row')
    if int(absolute_measured['bins_meeting_minimum_support']) != bins_meeting:
        raise PreflightError('the attested bin support disagrees with its gate row')
    if float(absolute_measured['maximum_absolute_ece']) != thresholds['maximum_absolute_ece']:
        raise PreflightError('the recorded absolute calibration verdict uses another threshold')
    if (
        float(delta_measured['maximum_ece_delta_vs_active'])
        != thresholds['maximum_ece_delta_vs_active']
    ):
        raise PreflightError('the recorded relative calibration verdict uses another threshold')

    return {
        'source': CALIBRATION_EVIDENCE_SOURCE_FROZEN,
        'pooled_ece': pooled_ece,
        'ece_delta_vs_active': ece_delta,
        'bins_meeting_minimum_support': bins_meeting,
        'reliability_bins_required': total_bins,
        'claim_supportable': claim_supportable,
        'attestation': {
            'source_artifact': _relative(V1_VALIDATION_RESULT_PATH),
            'source_byte_sha256': actual,
            'protocol_v2_custody_pin': pinned,
            'digest_re_derived_from_persisted_bytes': True,
            'protocol_v2_status': PROTOCOL_V2_STATUS,
            'pointers_read': list(CALIBRATION_ATTESTATION_POINTERS),
            'computed_from_the_frozen_bytes': True,
            'hardcoded_verdict': False,
            'validation_result_referenced_by_digest': True,
            'holdout_reopened_by_this_preflight': False,
            'validation_consumed_by_this_preflight': False,
            ...
            'candidate_canonical_payload_sha256': str(candidate['canonical_payload_sha256']),
            'candidate_detail_key': CALIBRATION_CANDIDATE_DETAIL_KEY,
            'active_reference_detail_key': CALIBRATION_ACTIVE_DETAIL_KEY,
            'measured': {
                'pooled_ece': pooled_ece,
                'active_reference_ece': active_ece,
                'ece_delta_vs_active': ece_delta,
                'bins_meeting_minimum_support': bins_meeting,
                'reliability_bins_required': total_bins,
                'bins_per_action_class': bins_per_action_class,
                'action_classes': action_classes,
                'claim_supportable_in_the_result': claim_supportable,
            },
            'recorded_gate_verdicts': {
                CALIBRATION_ABSOLUTE_GATE_ID: bool(absolute_row['pass']),
                CALIBRATION_DELTA_GATE_ID: bool(delta_row['pass']),
            },
        },
    }
...
def calibration_gate_decision(
    ...
    The production path attests the measurement from the frozen, digest-referenced
    v1 VALIDATION result (see :func:`attest_calibration_gate_evidence`), which is
    what ``source == CALIBRATION_EVIDENCE_SOURCE_FROZEN`` declares.  The function
    also accepts explicit caller-supplied evidence, which is how the synthetic
    tests exercise both the passing and the failing verdict.  Absent evidence the
    gate is ``evaluated: false`` and therefore refused -- an unevaluated layer-B
    gate may never close a node (``default_when_a_gate_is_unevaluated: false``).
    """
    ...
    if calibration_evidence is None:
        return {
            ...
            'evidence': {
                ...
                'reason': (
                    'no calibration evidence was supplied and the digest-referenced frozen '
                    'VALIDATION bytes were not attested, so the frozen absolute/relative '
                    'calibration gate stays open (fail closed)'
                ),
                'pooled_ece': None,
    ...
    # The attested path carries the frozen reliability-bin count and the
    # claim-supportability flag recorded in the VALIDATION bytes; when they are
    # present the gate enforces the full frozen requirement ("each reliability bin
    # carries at least minimum_bin_support_for_a_calibration_claim observations").
    required_bins = calibration_evidence.get('reliability_bins_required')
    if required_bins is not None:
        checks['every_reliability_bin_meets_the_minimum_support'] = bool(
            bins is not None and int(bins) >= int(required_bins)
        )
    if calibration_evidence.get('claim_supportable') is not None:
        checks['the_result_declares_the_calibration_claim_supportable'] = bool(
            calibration_evidence['claim_supportable']
        )
    evidence = {
    ...
    attestation = calibration_evidence.get('attestation')
    if isinstance(attestation, Mapping):
        evidence['attestation'] = dict(attestation)
    return {
        'evaluated': True,
...
def build() -> tuple[dict[str, Any], str]:
    ...
    with hand_history_tripwire() as opened:
        ...
        # The frozen v2 protocol is the authority that pins the re-derived v1
        # evidence, so the artifact records the custody pin it was checked against.
        v1_custody['protocol_v2_custody_pin'] = protocol_v2['v1_preflight_custody_pin']
        # The pin, the frozen bytes on disk and the frozen protocol custody row are
        # cross-checked together, so no one of the three can drift alone.
        v1_custody['pin_single_source'] = verify_custody_pin_agreement()
        calibration_thresholds = protocol_v2['calibration_thresholds']
        # The CALIBRATION gate is attested from the frozen, digest-referenced v1
        # VALIDATION bytes: no holdout is re-opened and no metric is recomputed.
        calibration_evidence = attest_calibration_gate_evidence(protocol_v2)
        calibration_verdict = calibration_gate_decision(
            calibration_evidence, calibration_thresholds
        )
    ...
    # The CALIBRATION gate verdict attested above is recorded verbatim (with its
    # evidence and attestation pointers), and the human-readable status derives
    # from that verdict instead of asserting a hardcoded "unevaluated" outcome.
    if calibration_verdict['evaluated']:
        calibration_gate_status = (
            'ATTESTED_FROM_FROZEN_V1_VALIDATION_BYTES_NO_NEW_HOLDOUT_READ: the frozen '
            'absolute/relative calibration gate is evaluated from the digest-referenced, '
            'already-consumed v1 VALIDATION result (source '
            f"{calibration_verdict['evidence']['source']}); verdict="
            f"{'PASS' if calibration_verdict['passed'] else 'FAIL'} -> "
            + (
                'the gate passes'
                if calibration_verdict['passed']
                else 'REFUSED_CALIBRATION, so every estimate node stays open'
            )
        )
    else:
        calibration_gate_status = (
            'UNEVALUATED_PRE_VALIDATION: no calibration evidence was attested, so the frozen '
            'absolute/relative calibration gate has no measured evidence and fails closed '
            '(REFUSED_CALIBRATION, default_when_a_gate_is_unevaluated=false)'
        )
    ...
    preflight = {
        ...
        'admissibility_protocol': {
            ...
            'calibration_gate': calibration_verdict,
...
def render_summary(preflight: Mapping[str, Any]) -> str:
    ...
    lines += [
        ...
        'before/after. The holdout is never re-opened: the frozen absolute/relative calibration '
        'gate is only *attested* from the published, digest-referenced VALIDATION result, so no '
        'hand history is parsed, no validation decision row is read, no metric is recomputed and '
        'no threshold is re-selected.',
        '',
        ...
        f"(`{preflight['frozen_v1_preflight']['bundle']}`, "
        f"`{preflight['frozen_v1_preflight']['artifacts'][NAME]}`) is the restored-custody v1 "
        'revision: byte-pinned, re-verified byte-for-byte on every run and never rewritten. Its '
        'pin, the frozen bytes persisted on disk and the frozen v2 protocol `v1_custody` row '
        f"(`{preflight['frozen_v1_preflight']['protocol_v2_custody_pin']}`) are one and the same "
        'digest, enforced by the single-source agreement recorded in '
        '`frozen_v1_preflight.pin_single_source`. A byte-identical regeneration stays impossible '
        'by construction because the v1 payload embeds the `code.preflight_tool.sha256` of the '
        'superseded v1 tool, which no longer exists on disk. The v2 bundle in '
        '`analysis/issue419_hierarchical_tree/exact_tree_preflight_v2/` supersedes v1 and is the '
        'only revision this tool writes (`--revision v2`; `--revision v1` is refused).',
        '',
        'Reproduce: `python3 tools/simulation/issue419_exact_tree_preflight.py --revision v2` '
        '(the default revision); verify: `python3 tools/simulation/issue419_exact_tree_preflight.py '
        '--check`. `--revision v1` is refused. This preflight records evidence only; it admits '
        'nothing.',
        '',
    ]
    return '\n'.join(lines)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default='v2',
        help=(
            'preflight revision to write.  ``v2`` (default) rewrites the content-addressed v2 '
            'bundle under analysis/issue419_hierarchical_tree/exact_tree_preflight_v2/.  ``v1`` '
            "is refused: the superseded v1 bundle is byte-pinned, re-verified and never "
            'rewritten'
        ),
    )
    parser.add_argument(
    ...
    if args.revision == 'v1':
        print(
            'REFUSED_REVISION_V1: the superseded v1 preflight bundle '
            f'({_relative(OUTPUT)}) is byte-pinned, re-verified and never rewritten; '
            'write the v2 revision instead (--revision v2)',
            file=sys.stderr,
        )
        return 2
    if args.check:
