...
``v1`` bundle (``exact_tree_preflight/``) is a frozen, pinned v1 surface: it is
re-verified byte-for-byte here and is never rewritten, and ``--revision v1`` is
refused outright.  v1 cannot be regenerated byte-identically by construction --
its payload records ``code.provider.sha256`` and ``code.preflight_tool.sha256``
of the superseded sources, and the recorded v1 tool digest no longer exists on
disk -- so the frozen bytes are kept and byte-pinned against the digests the
frozen v2 protocol custody table also records, with the supersession declared
explicitly in ``frozen_v1_preflight.supersession``.
"""
...
V1_PREFLIGHT_SHA256 = 'e86b7c6b57d170c0885f46734c41302dcc15608252dfbbd66bff62c258d5c65f'
V1_SUMMARY_SHA256 = '3b87d9bcc366c35e97bde6a4be350704a0cf2eecadb464d5ac321298443778df'
V1_INDEX_SHA256 = '4fea09fc5713b9fa490effb39c557962ae1d06146710d954fb21bfd39ef25743'
V1_BYTE_SHA256 = '69c99a8b37589f1687b7e980344c9a79d69bbcd45be0a53ffd59bfea0ab583b3'

# Explicit supersession declaration.  The v1 bundle records the v1 provider and
# the v1 preflight tool through ``code.provider.sha256`` /
# ``code.preflight_tool.sha256``, so its byte-identical regeneration stopped being
# possible the moment those sources changed: the v1 tool digest below no longer
# exists anywhere on disk (the tool was rewritten into this v2 revision).  v1 is
# therefore kept strictly byte-identical and only ever re-verified against the
# digests recorded below (and against the frozen v2 protocol custody table); it is
# never rewritten and never re-derived.  The v2 bundle in
# ``exact_tree_preflight_v2/`` supersedes it and is the only revision this tool
# writes.
V1_SUPERSESSION = {
    'declaration': (
        'SUPERSEDED_BY_V2_BYTE_PINNED_NOT_REGENERATED: the v1 preflight bundle is frozen, '
        'superseded evidence. Its recorded code.preflight_tool.sha256 names the superseded v1 '
        'tool, which no longer exists on disk, so a byte-identical re-derivation is impossible by '
        'construction; the bundle is byte-pinned to the digests recorded for it instead and is '
        'never rewritten.'
    ),
    ...
    'v1_records_provider_sha256': '7dfe162eef38e77277a6339555a3554cda104bbc256965734d17dff27707fdc7',
    ...
    'predecessor_note': (
        'byte pin of the v1 preflight before the provider-contract refresh re-derived and '
        're-pinned the T8 preflight (the frozen v2 protocol custody step); that revision is not '
        'on disk any more and is never restored, because its response payloads predate the '
        'current provider contract.  The byte pin kept and re-verified here is the current '
        'frozen v1 revision, and it is never rewritten by this tool.'
    ),
}
...
def render_summary(preflight: Mapping[str, Any]) -> str:
    ...
    lines += [
        ...
        f"`{preflight['frozen_v1_preflight']['artifacts'][NAME]}`) is byte-pinned: it is "
        're-verified byte-for-byte and never rewritten, and a byte-identical regeneration is '
        'impossible by construction because its payload embeds the `code.preflight_tool.sha256` '
        'of the superseded v1 tool, which no longer exists on disk. The pre-refresh v1 pin '
        f"(`{preflight['frozen_v1_preflight']['supersession']['predecessor_byte_sha256']}`) is "
        'recorded as history only and is never restored. The v2 bundle in '
