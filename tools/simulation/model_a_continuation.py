...
def _ordered_positions(
    players: Sequence[str], positions: Mapping[str, str], order: Sequence[str]
) -> list[str]:
    rank = {position: index for index, position in enumerate(order)}
    return sorted(
        (positions[player] for player in players),
        key=lambda position: (rank.get(position, 999), position),
    )
...
def _family(history: Sequence[Mapping[str, Any]], actor_position: str) -> str:
    ...
    actor_raised = any(
        row["position"] == actor_position and row["action"] in {"RAISE", "JAM"}
        for row in history
    )
    actor_called = any(
        row["position"] == actor_position and row["action"] in {"CALL", "LIMP"}
        for row in history
    )
    if len(raises) == 1:
        if limps and first > 0:
            actor_limped = any(
                row["position"] == actor_position and row["action"] == "LIMP"
                for row in history
            )
            if actor_limped:
    ...
    if len(raises) == 2:
        return (
            ...
            if actor_raised
            else "CALLER_VS_SQUEEZE_OR_3BET" if actor_called else "COLD_VS_3BET"
        )
    if len(raises) == 3:
        return (
            "AGGRESSOR_VS_4BET"
            if actor_raised
            else "CALLER_VS_4BET" if actor_called else "COLD_VS_4BET"
        )
    if len(raises) == 4:
        return "VS_5BET"
    return "VS_6BET_PLUS"
...
def _pot_type_and_role(
    preflop_history: Sequence[Mapping[str, Any]], actor: str
) -> tuple[str, str]:
...
def _preflop_decision(
    ...
    live = [
        player
        for player in replay.seats
        if not replay.folded[player] and not replay.all_in[player]
    ]
    all_in = [
        player
        for player in replay.seats
        if not replay.folded[player] and replay.all_in[player]
    ]
    clean_history = [
    ...
    return {
        ...
        "raise_level": sum(
            row["action"] in {"RAISE", "JAM"} for row in history
        ),
...
def _postflop_decision(
    ...
    *,
    ...
    ordered = sorted(
        active,
        key=lambda player: (
            POST_ORDER.index(positions[player])
            if positions[player] in POST_ORDER
            else 999,
            positions[player],
        ),
    )
...
def _semantic_trace(
    ...
    street_start = {
        "preflop": sum(not replay.all_in[player] for player in replay.live_players)
    }
    ...
    for event in state.action_log:
        ...
        while replay.street != target_street:
            ...
            street_start[replay.street] = sum(
                not replay.all_in[player] for player in replay.live_players
            )
...
def exact_preflop_node(
    model: Mapping[str, Any], decision: Mapping[str, Any]
) -> Mapping[str, Any] | None:
    matches = []
    wanted_history = [
        (str(row.get("position")), str(row.get("action")))
        for row in decision.get("history", [])
    ]
    for node in model.get("nodes", []):
        ...
        if (
            ...
            == str(decision.get("actor_position") or "")
            and int(context.get("table_size") or 0)
            == int(decision.get("table_size") or 0)
            and int(context.get("raise_level") or 0)
            == int(decision.get("raise_level") or 0)
            and str(context.get("family") or "")
            == str(decision.get("family") or "")
            and list(context.get("live_positions") or [])
            == list(decision.get("live_positions") or [])
            and list(context.get("all_in_positions") or [])
            == list(decision.get("all_in_positions") or [])
...
def _node_action_probability(
    ...
    if policy is not None:
        ...
        return max(
            0.0,
            float((policy.get(hand) or {}).get(semantic_action) or 0.0),
        )
...
def _semantic_legal(
    ...
    if semantic in {"LIMP", "CALL"}:
        if street == "preflop":
            expected = (
                "LIMP"
                if int(decision.get("raise_level") or 0) == 0
                else "CALL"
            )
            return "CALL" in legal and semantic == expected
        return "CALL" in legal and semantic == "CALL"
    if semantic == "JAM":
    ...
    if semantic == "BET":
        return (
            street != "preflop"
            and "RAISE" in legal
            and str(decision.get("mode") or "") == "FREE"
        )
    if semantic == "RAISE":
        if street == "preflop":
            return "RAISE" in legal
        return (
            "RAISE" in legal
            and str(decision.get("mode") or "") == "FACING"
        )
    return False
...
def _empirical_raise_target(
    node: Mapping[str, Any], state: NoLimitHoldemState, actor: str
) -> tuple[float, str]:
    ...
    containers = [
        node.get("continuous") or {},
        (node.get("population_model") or {}).get("continuous") or {},
    ]
    target = None
    ...
    if target is None:
        target = (
            ...
            else min(maximum, current + state.big_blind_bb)
        )
    target = min(
        maximum,
        max(float(target), minimum if minimum is not None else current + EPS),
    )
    if target <= current + EPS:
        raise ModelAUnsupportedContext(
            "exact node has no translatable legal raise sizing"
        )
...
class ModelAContinuationPolicy:
    ...
    def __init__(
        self,
        preflop_model: Mapping[str, Any],
        postflop_model: Mapping[str, Any],
        *,
    ...
    def from_paths(
        cls,
        preflop_path: Path,
        postflop_base_path: Path,
        continuation_overlay_path: Path,
        ...
        if str(overlay.get("base_postflop_model_sha256") or "") != actual_base:
            raise ValueError(
                "#102 continuation overlay does not match the supplied postflop baseline"
            )
        ...
        return cls(
            ...
            identity={
                ...
                "continuation_overlay_sha256": _sha256(
                    continuation_overlay_path
                ),
                "stage_b_decision": (overlay.get("selection") or {}).get(
                    "stage_b_decision"
                ),
            },
        )
    ...
    def _posterior_from_history(
        ...
        for item in trace:
            ...
            if item["street"] == "preflop":
                ...
                if node is None:
                    raise ModelAUnsupportedContext(
                        "missing exact preflop Model-A node in prior history"
                    )
            ...
            action_index = actions.index(semantic)
            weights = _normalize_weights(
                [
                    weight * max(1e-12, matrix[index][action_index])
    ...
    def action_probabilities(
        ...
        **_: Any,
        ...
        if len(hole_cards) != 2:
            raise ValueError(
                "Model A requires exactly two acting-player hole cards"
            )
        trace, replay, preflop_history, street_histories, street_start = (
            _semantic_trace(state)
        )
        if replay.to_snapshot(include_log=False) != state.to_snapshot(
            include_log=False
        ):
            raise AssertionError(
                "semantic replay diverged from NoLimitHoldemState"
            )
        view = state.legal_view(actor)
        if state.street == "preflop":
            ...
            if node is None:
                raise ModelAUnsupportedContext(
                    "missing exact preflop Model-A node"
                )
            ...
            if raw is None:
                raw = (
                    (node.get("population_model") or {}).get("frequencies")
                    or {}
                )
            ...
            if node is None:
                raise ModelAUnsupportedContext(
                    "missing exact postflop Model-A node"
                )
            combos, weights = self._posterior_from_history(
                state, actor, trace
            )
            actual = tuple(
                sorted((cid(hole_cards[0]), cid(hole_cards[1])))
            )
            try:
                combo_index = combos.index(actual)
            except ValueError as exc:
                raise ModelAUnsupportedContext(
                    "acting player's dealt combo is outside reconstructed Model-A support"
                ) from exc
            target = target_frequencies(
                node, decision, self.postflop_model
            )
            actions, matrix = calibrated_action_matrix(
                combos,
                weights,
    ...
    def decide(
        ...
        **context: Any,
        ...
        if semantic in {"LIMP", "CALL"}:
            ...
            core = "RAISE"
            if state.street == "preflop":
                node = exact_preflop_node(
                    self.preflop_model, info["semantic_context"]
                )
            else:
                node = exact_postflop_node(
                    self.postflop_model, info["semantic_context"]
                )
            if node is None:
                raise ModelAUnsupportedContext(
                    "selected raise lost exact-node support"
                )
