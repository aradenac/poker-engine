...
It also documents the two supersessions the issue asks for: the superseded v2
protocol payload ``508a31ec...`` (replaced by amendment
``V2_AMENDMENT_1_CONDITIONAL_NODE_CLOSURE``, current payload ``74b8a006...``)
and the ``v1 EXACT_TREE_PREFLIGHT.json`` regeneration (mutant commit ``871e0bd``
rewrote the frozen bytes to ``e86b7c6b...``; the evidence-integrity correction
``c904524`` restored ``456d85be...``).  The residual consequence is reported
rather than hidden or "repaired": the preflight tool's ``V1_PREFLIGHT_SHA256`` /
``V1_INDEX_SHA256`` pins were re-pointed to the pre-correction digests by commit
``887e46a``, so ``issue419_exact_tree_preflight.py --check`` is RED today.  That
defect is owned by the preflight task; repairing it here would move a frozen
surface pinned by the T8 preflight and the T9 terminal decision, which this task
is forbidden to do.
...
* ``FROZEN_VALIDATION_PROTOCOL_V2.json`` / ``VALIDATION_RESULT.json`` -- digest
  references (no bytes duplicated, no holdout re-read)
...
* ``ARTIFACTS.json`` -- content-addressed index (its own eleven members)
...
PREFLIGHT_V2_BYTE_SHA256 = '2c07ae3279d10ec3e3bbdbe81cdb0d0f1f23e6bbb91cbdadddae113d1a9ea691'
PREFLIGHT_V2_CANONICAL_SHA256 = 'c25afda4d1fd56608b5041ba3cd63749b8229f10ea980a6f8957d02d52d24bcb'
...
DECISION_V2_BYTE_SHA256 = 'ac291b9ed363080401e9649a30b157eec7f7b79d5d2dc6df70390ccebc67c4b5'
DECISION_V2_CANONICAL_SHA256 = 'c6accf274293dc60f7df016bb4f4d5e5ff62a86b91e63f719187424ff803f037'
...
def preflight_pin_findings() -> list[dict[str, Any]]:
    """Machine-derived residual findings: the preflight pins are stale.

    Commit ``887e46a`` re-pointed the exact-tree preflight tool's frozen pins to
    the digests the evidence-integrity correction had already reverted: the v1
    preflight payload/index and the protocol v2 bytes.  Both mismatches are
    computed here from the persisted bytes and the tool's own constants, and both
    stay unfixed for the same reason -- the tool digest is pinned by the T8
    preflight payload and by the T9 terminal decision.
    """
    ...
    not_repaired = (
        'the preflight tool digest is pinned by the T8 preflight payload and by the T9 '
        'terminal decision, so editing it would rewrite frozen evidence this task may '
        'not touch'
    )
    v1 = {
        'finding_id': 'V1_PREFLIGHT_PIN_STALE',
        'detail': (
            'the preflight tool pins the pre-correction v1 preflight bytes, which the '
            'evidence-integrity correction restored to a different (authoritative) digest, so '
            'issue419_exact_tree_preflight.py --check fails closed on its own frozen surface'
        ),
        'detected_by': (
            'python3 tools/simulation/issue419_exact_tree_preflight.py --check'
        ),
        'observed': 'PreflightError: the frozen v1 preflight artifact drifted',
        ...
        'owner': 'the exact-tree preflight task; repairing it here would move a frozen surface',
        'repaired_by_this_bundle': False,
        'blocks_ready_for_integration': True,
        'reason_not_repaired_here': not_repaired,
    }
    v2 = {
        'finding_id': 'PROTOCOL_V2_PREFLIGHT_PIN_STALE',
        'detail': (
            'the same preflight tool also pins the pre-correction protocol v2 bytes, so the '
            'protocol-v2 custody check inside the preflight suite fails even after the v1 pin '
            'is repaired'
        ),
    ...
    with hand_history_tripwire() as opened_paths:
        ...
        required_supersessions = [
            ...
            },
            {
                ...
                'residual_finding': 'V1_PREFLIGHT_PIN_STALE',
                'residual_findings': [
        ...
        recorded_local_observations = [
            ...
            },
            {
                ...
                'observed_result': (
                    'FAIL: the frozen v1 preflight artifact drifted: EXACT_TREE_PREFLIGHT.json '
                    f'{PREFLIGHT_V1_BYTE_SHA256} != {PREFLIGHT_V1_PRECORRECTION_BYTE_SHA256}'
                ),
                'observed_exit_code': 1,
                'explained_by': 'V1_PREFLIGHT_PIN_STALE',
            },
            {
            ...
            },
            {
                ...
                'observed_result': (
                    'FAIL: 5 failures, 7 errors; the suite re-derives the pre-correction v1 '
                    'preflight and protocol v2 digests the tool still pins'
                ),
                'observed_exit_code': 1,
                'explained_by': 'V1_PREFLIGHT_PIN_STALE,PROTOCOL_V2_PREFLIGHT_PIN_STALE',
            },
        ]
        ...
        bundle = {
            ...
            'provenance': {
                ...
                'raise_sizing_frontier_resolution': _relative(FRONTIERS),
...
def render_report(bundle: Mapping[str, Any]) -> str:
    ...
    lines.append('`INTEGRATION_REPORT.json` is the machine-readable source of this document; '
                 '`INTEGRATION_REPORT.md` is rendered from it. `ARTIFACTS.json` binds all eleven '
                 'members by byte digest and content-addressed object, and '
    ...
    for item in bundle['required_supersessions']:
        ...
        if 'superseded' in item and isinstance(item['superseded'], dict):
            ...
            lines.append(f'* superseded by `{item["superseded_by"]}`; residual finding '
                         f'`{item["residual_finding"]}`')
        lines.append('')
    ...
    lines.append('')
    lines.append('## 5. Residual findings')
    lines.append('')
    _table(
        lines,
    ...
    finding = next(item for item in bundle['residual_findings']
    ...
    lines.append(f'`V1_PREFLIGHT_PIN_STALE` in detail: the preflight tool pins '
                 ...
                 f'`V1_INDEX_SHA256={finding["tool_pins"]["V1_INDEX_SHA256"]}`, while the restored '
                 ...
                 f'The pin was re-pointed by `{finding["introduced_by"]}` after the correction '
                 f'`{finding["restored_by"]}` restored the bytes, so `{finding["detected_by"]}` is '
                 f'red. It is **not** repaired here ({finding["reason_not_repaired_here"]}); it is '
                 f'owned by {finding["owner"]}.')
    lines.append('')
    v2_finding = next(item for item in bundle['residual_findings']
    ...
    lines.append(f'`PROTOCOL_V2_PREFLIGHT_PIN_STALE` in detail: the same tool pins '
                 ...
                 f'{v2_finding["tool_pins"]["PROTOCOL_V2_CANONICAL_PAYLOAD_SHA256"]}`, while the '
                 ...
                 f'{str(v2_finding["pin_matches_on_disk_bytes"]).lower()}`). This is the pin the '
                 f'preflight suite trips first (`{v2_finding["detected_by"]}`), and it is the same '
                 f'staleness as the v1 pin: `{v2_finding["introduced_by"]}` re-pointed both to the '
                 f'pre-correction digests. It is not repaired here either, for the same reason.')
    lines.append('')
    ...
    lines.append('1. the `V1_PREFLIGHT_PIN_STALE` and `PROTOCOL_V2_PREFLIGHT_PIN_STALE` defects '
                 'fixed by their owner task, so `issue419_exact_tree_preflight.py --check` and the '
                 'preflight suite are green again without moving a frozen surface;')
...
def render_summary(bundle: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f'`{PREFLIGHT_V1_BYTE_SHA256}` by correction `{CORRECTION_COMMIT}`, leaving the residual '
        '`V1_PREFLIGHT_PIN_STALE` and `PROTOCOL_V2_PREFLIGHT_PIN_STALE` findings documented in '
        '`INTEGRATION_REPORT.md` §5.',
        '',
        '```',
...
def reference_document(name: str, bundle: Mapping[str, Any]) -> dict[str, Any]:
    """The two digest-reference members, built from the verified authorities."""
    protocol_v2 = _load(PROTOCOL_V2)
