...
In target-population mode, the hand-conditioned ``policy169_q_b64`` representation
preferred by the browser may be refitted. Revealed cards are used only when they
were really observed; hidden hands are never imputed.
"""
...
def rebuild(*, baseline: Path, overlay_path: Path, decisions: Path, out: Path,
    ...
    for node in obj.get("nodes",[]):
        ...
        if evidence_mode == "replace_population":
            ...
            if node.get("policy169_q_b64"):
                policy_nodes_seen+=1
                before=str(node["policy169_q_b64"])
            ...
            for part in ("target_population_refit_v1", "policy169_refit_v1" if policy169_update_weight > 0 and node.get("policy169_q_b64") else None):
                if part and f"+{part}" not in current_method:
                    current_method+=f"+{part}"
            model["method"]=current_method
        else:
            # Keep this block byte-for-byte equivalent in semantics and arithmetic
            # to the historical implementation. Closed-run reproducibility depends
            # on exact floating-point operation order and core coverage semantics.
            new_n=old_n+dn
            for action in legal:
                dc=int(delta_actions.get(action) or 0)
    ...
    if evidence_mode == "incremental":
        # Preserve the closed-run metadata contract too; newer diagnostics belong
        # only to the explicit target-population path.
        ...
        policy_mode=(
            ...
            else "REFIT from TRAIN marginals plus revelation-rate-shrunk observed hand/action residuals"
        )
        obj["incremental_update"]={
...
def main():
    ...
    p.add_argument("--policy169-update-weight",type=float,default=0.0,help="0 preserves historical bytes; #101 target candidates use a positive weight")
