#!/usr/bin/env python3
"""Build normalized Model A population decisions from an immutable hand-history increment."""
...
def amount(s):
    ...
    if "," in s and "." in s:
        ...
        p = s.split(",")
        s = p[0] + "." + p[1] if len(p) == 2 and len(p[1]) != 3 else s.replace(",", "")
    try:
        return float(s)
    except ValueError:
        return float("nan")
...
def pos_order(p):
    try:
        return ["LJ", "HJ", "CO", "BTN", "SB", "BB", "SB_BTN"].index(p)
    except ValueError:
        return 99
...
def post_pos_order(p):
    try:
        return ["SB", "BB", "LJ", "HJ", "CO", "BTN", "SB_BTN"].index(p)
    except ValueError:
        return 99


def assign_positions(seats, button):
    ss = sorted(seats, key=lambda x: x["seat"])
    out = {}
    ...
    for i, s in enumerate(ordered):
        out[s["name"]] = templ[i] if i < len(templ) else f"Seat {s['seat']}"
    return out
...
def family(history, actor):
    ...
    if not raises:
        return "VS_LIMPERS"
    first = raises[0]
    callers = [x for x in history[first + 1 :] if x["action"] == "CALL"]
    actor_raised = any(x["position"] == actor and x["action"] in ("RAISE", "JAM") for x in history)
    actor_called = any(x["position"] == actor and x["action"] in ("CALL", "LIMP") for x in history)
    if len(raises) == 1:
        if limps and first > 0:
            actor_limped = any(x["position"] == actor and x["action"] == "LIMP" for x in history)
            if actor_limped:
...
def board_features(board, prior=None):
    ...
    paired = 1 if any(v >= 2 for v in cnt.values()) else 0
    trips = 1 if any(v >= 3 for v in cnt.values()) else 0
    ...
    prior = prior or board[:-1]
    pr = [RANKS.index(c[0].upper()) for c in prior]
    ...
    return {
        ...
        "high_rank_norm": high,
        "broadway_norm": broad,
        "new_overcard": 1 if nr is not None and pr and nr > max(pr) else 0,
        "new_pairs_board": 1 if nr is not None and ranks.count(nr) >= 2 else 0,
    }
...
def parse_hand(text, source):
    ...
    if not hm:
        return None
    hid = hm.group(1)
    bm = re.search(r"Seat #(\d+) is the button", text)
    button = int(bm.group(1)) if bm else None
    seats = []
    for line in lines:
        m = re.match(r"^Seat (\d+): (.+?) \(([^)]+?) in chips\)\s*$", line)
        if m:
            seats.append({"seat": int(m.group(1)), "name": m.group(2), "chips": amount(m.group(3))})
    dealt = re.search(r"^Dealt to (.+?) \[([^\]]+)\]", text, re.M)
    if not dealt:
        return None
    hero = dealt.group(1)
    known = {hero: dealt.group(2).split()}
    seatname = {s["seat"]: s["name"] for s in seats}
    for line in lines:
        m = re.match(r"^(.+?): shows \[([^\]]+)\]", line)
        if m:
            known[m.group(1).strip()] = m.group(2).split()
        m = re.match(r"^Seat (\d+): .*?\b(?:showed|mucked) \[([^\]]+)\]", line)
        if m and int(m.group(1)) in seatname:
            known[seatname[int(m.group(1))]] = m.group(2).split()
    ...
    if not bb:
        return None
    players = [{**s, "pos": pos.get(s["name"], ""), "known": known.get(s["name"])} for s in seats]
    by = {p["name"]: p for p in players}
    streets = {"preflop": [], "flop": [], "turn": [], "river": []}
    current = "preflop"
    pot = 0.0
    paid = collections.defaultdict(float)
    boards = {"preflop": [], "flop": [], "turn": [], "river": []}
    full = []
    for line in lines:
        if line.startswith("*** FLOP ***"):
            current = "flop"; paid = collections.defaultdict(float)
            m = re.search(r"\[([^\]]+)\]", line); full = m.group(1).split() if m else []; boards[current] = full.copy(); continue
        if line.startswith("*** TURN ***"):
            current = "turn"; paid = collections.defaultdict(float)
            m = re.findall(r"\[([^\]]+)\]", line); full = (m[0].split() + m[1].split()) if len(m) >= 2 else full; boards[current] = full.copy(); continue
        if line.startswith("*** RIVER ***"):
            current = "river"; paid = collections.defaultdict(float)
            m = re.findall(r"\[([^\]]+)\]", line); full = (m[0].split() + m[1].split()) if len(m) >= 2 else full; boards[current] = full.copy(); continue
        if line.startswith("*** SHOW DOWN ***") or line.startswith("*** SUMMARY ***"):
            continue
        m = re.match(r"^Uncalled bet \(([^)]+)\) returned to (.+)$", line)
        if m:
            a = amount(m.group(1)); pl = m.group(2).strip(); pot = max(0, pot - a); paid[pl] = max(0, paid[pl] - a)
            streets[current].append({"player": pl, "type": "return", "raw": a, "pot_after": pot, "allin": False}); continue
        ...
        if m:
            pl = m.group(1); a = amount(m.group(2)); pot += a; paid[pl] += a
            streets[current].append({"player": pl, "type": "call", "raw": a, "pot_after": pot, "allin": "and is all-in" in line}); continue
        m = re.match(r"^(.+?): bets ([€$£]?\s*[\d.,]+)(?: and is all-in)?$", line)
        if m:
            pl = m.group(1); a = amount(m.group(2)); pot += a; paid[pl] += a
            streets[current].append({"player": pl, "type": "bet", "raw": a, "pot_after": pot, "allin": "and is all-in" in line}); continue
        m = re.match(r"^(.+?): raises ([€$£]?\s*[\d.,]+) to ([€$£]?\s*[\d.,]+)(?: and is all-in)?$", line)
        if m:
            pl = m.group(1); to = amount(m.group(3)); add = max(0, to - paid[pl]); pot += add; paid[pl] = to
            streets[current].append({"player": pl, "type": "raise", "raw": add, "pot_after": pot, "allin": "and is all-in" in line}); continue
        m = re.match(r"^(.+?): checks\b", line)
        if m:
            streets[current].append({"player": m.group(1), "type": "check", "raw": 0, "pot_after": pot, "allin": False}); continue
        m = re.match(r"^(.+?): folds\b", line)
        if m:
            streets[current].append({"player": m.group(1), "type": "fold", "raw": 0, "pot_after": pot, "allin": False}); continue
...
def decision_rows(h):
    ...
    for a in h["streets"]["preflop"]:
        if a["type"] == "post":
            ...
            continue
        if a["type"] not in ("fold", "check", "call", "raise", "bet") or a["player"] not in by: continue
        actor = norm_pos(by[a["player"]]["pos"], n)
        act = "FOLD" if a["type"] == "fold" else "CHECK" if a["type"] == "check" else (("LIMP" if rl == 0 else "CALL") if a["type"] == "call" else ("JAM" if a["allin"] else "RAISE"))
    ...
    for st in ("flop", "turn", "river"):
        ...
        for a in acts:
            ...
            if a["type"] not in ("fold", "check", "call", "bet", "raise") or a["player"] not in by: continue
            ...
            row = {
                "hand_id": h["id"], "split": split, "street": st, "player": a["player"], "is_hero": a["player"] == hero, "action": act, "mode": mode,
                ...
                "board": board, "board_features": bf, "pot_before_bb": pot_before, "to_call_bb": tocall,
                "facing_price_pot": tocall / pot_before if pot_before > 0 else 0,
                "own_size_pot": add / pot_before if act in ("BET", "RAISE", "JAM") and pot_before > 0 else None,
                "spr": rem / pot_before if pot_before > 0 else 0, "known_hand_class": hand_class(by[a["player"]].get("known")), "known_cards": by[a["player"]].get("known"),
            }
            rows.append(row)
...
def build(source: Path, out: Path, summary_path: Path):
    ...
    stats = {
        "hands": collections.Counter(), "decisions": collections.Counter(), "population_actions": collections.Counter(),
        "hero_actions": collections.Counter(), "known_population_decisions": collections.Counter(),
        "preflop_nodes": collections.defaultdict(set), "postflop_nodes": collections.defaultdict(set),
    }
    parse_errors = []; allrows = []
    for record in records:
        h = parse_hand(record.text, record.source_file)
        if not h:
    ...
    out.parent.mkdir(parents=True, exist_ok=True); summary_path.parent.mkdir(parents=True, exist_ok=True)
    with out.open("w", encoding="utf-8") as f:
        for r in allrows: f.write(json.dumps(r, ensure_ascii=False, separators=(",", ":")) + "\n")
    summary = {
        "schema": "poker-population-increment-stats/v2", "source": source.name,
        "source_sha256": source_meta["sha256"], "source_hand_ids_fingerprint_sha256": fingerprint(r.hand_id for r in records),
        "source_unique_hands": source_meta["unique_hands"], "parsed_hands": sum(stats["hands"].values()), "parse_errors": parse_errors,
        ...
        "distinct_postflop_nodes": {k: len(v) for k, v in stats["postflop_nodes"].items()}, "rows_total": len(allrows),
        "provenance": {"builder": Path(__file__).relative_to(ROOT).as_posix(), "builder_sha256": sha256_file(Path(__file__)), "code_commit": git_commit(), "split_namespace": "poker-population-split-v1"},
    }
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps(summary, ensure_ascii=False, indent=2)); print("WROTE", out, os.path.getsize(out), "bytes"); print("WROTE", summary_path, os.path.getsize(summary_path), "bytes")
    return summary
