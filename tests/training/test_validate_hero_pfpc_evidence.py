...
def evidence_fixture(root: Path, *, bad_sizing_position: str | None = None):
    raw = root / "raw"
    paths = {
    ...
    return evidence, raw / "BTN"


def test_valid_five_position_evidence_passes_and_checks_decision_parity():
    with tempfile.TemporaryDirectory() as tmp:
        evidence, btn = evidence_fixture(Path(tmp))
        result = validate(evidence, btn)
        ...
        assert result["positions"]["BTN"]["parity_checked_hands"] == 0
...
def test_selected_sizing_drift_fails_even_when_all_hashes_are_self_consistent():
    with tempfile.TemporaryDirectory() as tmp:
        evidence, btn = evidence_fixture(Path(tmp), bad_sizing_position="CO")
        try:
            validate(evidence, btn)
