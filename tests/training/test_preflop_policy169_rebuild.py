...
def test_target_positive_weight_changes_runtime_consumed_policy():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        before_blob = runtime_policy(before_node)
        ...
        assert runtime_policy(node) != before_blob
        assert runtime_policy(node)["method"].endswith("+policy169_refit_v1")
