...
def node_from_policy(policy):
    return {
        "policy169_q_b64": encode_policy169(policy, actions=ACTIONS, hand_grid=GRID),
        "policy169_q_meta": {
            "actions": ACTIONS,
            "hand_grid": GRID,
            "scale": DEFAULT_SCALE,
...
def test_encoding_is_action_major_uint16_little_endian():
    node = node_from_policy(baseline_policy())
    raw = base64.b64decode(node["policy169_q_b64"])
    values = struct.unpack("<" + "H" * (len(ACTIONS) * len(GRID)), raw)
    assert len(values) == len(ACTIONS) * len(GRID)
    # Every hand distribution is quantized to exactly one scale unit even
    # though values are serialized action-major.
...
def test_revealed_evidence_changes_hand_conditioned_probability_without_imputation():
    node = node_from_policy(baseline_policy())
    delta = {
        "n_delta": 80,
        "actions": {"FOLD": 40, "CALL": 20, "RAISE": 20},
        # Only actually shown hands enter this table. Hidden FOLD/CALL rows do
        # not receive invented classes.
        "known_by_action": {
            "RAISE": {"AA": 8, "AKs": 2},
            "CALL": {"AKs": 2},
        },
    ...
    neutral_encoded, _neutral_diag = refit_node_policy169(
        node,
        no_reveal_delta,
        root_hand_meta=META,
        fallback_grid=GRID,
    ...
    neutral_node = dict(node)
    neutral_node["policy169_q_b64"] = neutral_encoded
    candidate = dict(node)
    candidate["policy169_q_b64"] = encoded
    ...
    # The action marginals are fixed by the same delta in both refits. The
    # revealed-card residual should therefore be tested against the matched
    # no-reveal refit, not against the raw baseline policy whose marginals differ.
    assert after["AA"]["RAISE"] > neutral["AA"]["RAISE"]
    assert after["72o"]["RAISE"] < neutral["72o"]["RAISE"]


def test_zero_update_weight_preserves_blob_exactly():
    node = node_from_policy(baseline_policy())
    delta = {
        "n_delta": 20,
    ...
    encoded, _diag = refit_node_policy169(
        node,
        delta,
        root_hand_meta=META,
        fallback_grid=GRID,
    ...
    assert encoded == node["policy169_q_b64"]


def test_malformed_payload_is_rejected():
    node = node_from_policy(baseline_policy())
    node["policy169_q_b64"] = base64.b64encode(b"short").decode("ascii")
    try:
