...
function testCanonicalContextPreventsArbitraryFourBetLabel(){
  const c=context();
  assert.equal(c.family,'UNOPENED');
  assert.equal(Search.semanticRaiseAction(c),'OPEN');
  assert.equal(Search.semanticAction('RAISE',c),'OPEN');
  assert(!Search.semanticLegalActions(c).includes('4BET'));
}
