...
def test_current_cycle_snapshot_is_ready_with_all_candidates_retained() -> None:
    ...
    for name in (
        ...
        assert by_name(report, name)["status"] == "PASS", (name, by_name(report, name))
    for name in ("model_a_preflop", "model_a_postflop", "model_b", "strategy"):
        component = by_name(report, name)
